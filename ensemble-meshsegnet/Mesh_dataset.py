...
class Mesh_Dataset(Dataset):
    def __init__(self, data_list_path, num_classes=15, patch_size=7000):
        ...
        self.data_list = glob.glob(data_list_path + '*.mat')
    ...
    def __len__(self):
        ...
        return len(self.data_list) #self.data_list.shape[0]

    def __getitem__(self, idx):
        #if torch.is_tensor(idx):

        data_mat = scio.loadmat(self.data_list[idx])
        data_mat_in = data_mat['in']
        X = data_mat_in[0, :, :]
        Y = data_mat_in[1, :, :]
        print(X.shape)
        print(Y.shape)
        ...
        # read vtk
        mesh = load(i_mesh)
        print(mesh.getCellArray('labels'))
