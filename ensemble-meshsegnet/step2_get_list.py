...
if __name__ == '__main__':
    ...
    valid_sample_list = []
    for i_sample in sample_list:     # 检查'./augmentation_vtk_data/'文件夹中有多少个'A{0}_Sample_0{1}_d.vtp'文件
        for i_aug in range(num_augmentations):
            if os.path.exists(os.path.join(data_path, sample_name.format(i_aug, i_sample))):
                valid_sample_list.append(i_sample)   # 将文件的sample标签传入list，例如[i_sample1(i_aug1个),i_sample2(i_aug2个）....]

    # remove duplicated
    sample_list = list(dict.fromkeys(valid_sample_list))  # sample_list[i_sample_1,i_sample2,.....,]
    sample_list = np.asarray(sample_list) # 将list格式转换为ndarray
    #print(sample_list)

    i_cv = 0
    kf = KFold(n_splits=3, shuffle=False)  # 6重交叉验证，随机关闭
    for train_idx, test_idx in kf.split(sample_list):  # 6重分割，5份训练1份验证，训练存在train_idx中，测试存在test_idx中

        i_cv += 1
        print('Round:', i_cv)

        train_list, test_list = sample_list[train_idx], sample_list[test_idx]
        train_list, val_list = train_test_split(train_list, train_size=0.8, shuffle=True) # 将训练数据4：1存入train_list和val_list中，打乱
        ...
        for i_sample in train_list:
            for i_aug in range(num_augmentations):
                ...
                train_name_list.append(os.path.join(data_path, subject_name)) # 将文件地址及文件名存入train_name_list
                if with_flip:
