...
if __name__ == "__main__":
    ...
    vtk_path = 'F:\\biyesheji\MeshSNet\dentalmesh(1)\dentalmeshvtp'# need to define the path; e.g., src
    output_save_path = './augmentation_vtk_data'
    if not os.path.exists(output_save_path):
        os.mkdir(output_save_path)

    sample_list = list(range(1, num_samples+1))#列表sample_list[1,2,3,....,num_samples+1]
    num_augmentations = 20

    for i_sample in sample_list:
        for i_aug in range(num_augmentations):

            file_name = 'Sample_0{0}_d.vtp'.format(i_sample)#文件名Sample_0(1、2....）_d.vtp
